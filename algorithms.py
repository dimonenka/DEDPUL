...
def estimate_poster_en(preds, target, alpha=None, estimator='e1', quantile=0.05, **kwargs):
    ...
    if alpha is None:
        if estimator == 'e1':
            ...
            alpha = 1 - min(np.quantile(preds[target == 1] / (1 - preds[target == 1]),
                                        quantile, interpolation='lower'), 1)
        alpha = max(alpha, 0)
...
def estimate_poster_cv(df, target, estimator='dedpul', alpha=None, estimate_poster_options=None,
    ...
    if estimator == 'dedpul':
        ...
        res['em_en'] = estimate_poster_em(preds=preds, target=target, mode='en', alpha=alpha, **estimate_poster_options)
        return res
